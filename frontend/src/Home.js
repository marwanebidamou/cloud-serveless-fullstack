...
// New keyframes for zooming in and translating left to right
const zoomInTranslate = keyframes`
  0% { 
...
const modalStyle = {
    ...
    p: 4,
    overflowY: 'auto',
    animation: `${zoomInTranslate} 1s ease-in-out`,  // Updated animation
};
...
function Home() {
    ...
    const convertImageToBase64 = (file) => {
        return new Promise((resolve, reject) => {
            ...
            reader.onloadend = () => {
                resolve(reader.result); // This gives you the Base64 string
            };
    ...
    const handleSubmit = async (e) => {
        ...
        if (image) {
            try {
                ...
                const uploadUrl = imageResponse.data.uploadUrl;
                const base64data = await convertImageToBase64(image);
                // Step 2: Upload the file directly to the provided URL
                ...
                // Step 3: Use the file URL returned by the server
                imageUrl = imageResponse.data.fileUrl;

                // Step 4: Update localStorage.user
                const nuser = JSON.parse(localStorage.getItem('user')); // Get the current user object
                nuser.profileImageUrl = imageUrl; // Update the profileImageUrl
                localStorage.setItem('user', JSON.stringify(nuser)); // Save it bac
            } catch (error) {
